"""
Adversarial Attack Results — XGBoost / Credit Card Fraud 2023
Generates:
...
ATTACKS = {
    "HopSkipJump":     "hop_skip_jump_xgboost",
    "Boundary Attack": "boundary_attack_xgboost",
    "Square Attack":   "square_attack_xgboost",
}
...
for name, (X, X_adv, Y, evals) in data.items():
    ...
    predicted = np.argmax
    # sample-level success: X_adv differs from X (perturbation applied)
    # We recompute from raw arrays for accuracy
    linf = linf_per_sample(X, X_adv)

    # a sample is a successful evasion only if the model actually changed prediction
    # We approximate using the aggregate ASR and per-sample linf distribution
    asr_total = evals[key]["is_misclassified_rate"]
    n = len(X)

    # cumulative evasions: how many samples have linf <= budget AND are evaded
    # We assume evaded samples have the smallest perturbations (best case for the attacker)
...
ax1.set_title("XGBoost — Evasiones acumuladas vs presupuesto L∞\nCreditcard Fraud 2023 (n=2000)", fontsize=13)
...
plt.tight_layout()
fig1.savefig(os.path.join(SAVE, "xgboost_cumulative_evasions.png"), dpi=150)
print("\nGuardado: xgboost_cumulative_evasions.png")
...
ax2.set_title("XGBoost — Comparativa de métricas por ataque\nCreditcard Fraud 2023 (n=2000)", fontsize=13)
ax2.legend(fontsize=11)
ax2.grid(True, axis="y", alpha=0.3)
plt.tight_layout()
fig2.savefig(os.path.join(SAVE, "xgboost_metrics_comparison.png"), dpi=150)
print("Guardado: xgboost_metrics_comparison.png")
...
for name, (X, X_adv, Y, evals) in data.items():
    ...
    rows.append({
        ...
        "L∞ estd. medio":     f"{e['stand_linf_costs_mean']:.3f}",
        "L∞ estd. (evasiones)": f"{e['stand_linf_costs_on_mis_mean']:.3f}",
    })

df = pd.DataFrame(rows)
print("\n── Tabla de resultados XGBoost ──────────────────────────────────")
print(df.to_string(index=False))

# ── 3b. TABLE AS FIGURE ───────────────────────────────────────────────────────
fig3, ax3 = plt.subplots(figsize=(11, 2.2))
ax3.axis("off")
...
ax3.set_title("XGBoost — Credit Card Fraud 2023  |  n = 2000 muestras",
              fontsize=12, fontweight="bold", pad=12)
plt.tight_layout()
fig3.savefig(os.path.join(SAVE, "xgboost_results_table.png"), dpi=150, bbox_inches="tight")
print("Guardado: xgboost_results_table.png")
