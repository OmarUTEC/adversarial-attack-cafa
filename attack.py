...
def main(cfg: DictConfig) -> None:
    ...
    if cfg.ml_model.perform_training or cfg.ml_model.perform_grid_search_hparams:
        if cfg.ml_model.model_type not in MODEL_TRAINERS:
            raise ValueError(f"Unsupported model_type={cfg.ml_model.model_type}")
        model_trainers = MODEL_TRAINERS[cfg.ml_model.model_type]
        best_hparams = cfg.ml_model.default_hparams
        if cfg.ml_model.perform_grid_search_hparams:
            best_hparams = model_trainers['grid_search'](trainset=trainset,
                                                         testset=devset,
                                                         tab_dataset=tab_dataset)
        model_trainers['train'](best_hparams, trainset=trainset, testset=devset, tab_dataset=tab_dataset,
    ...
    should_use_constraints = (
        'constraints' in cfg and cfg.constraints and
        (cfg.perform_projection or cfg.perform_constraints_soundness_evaluation)
    )
    if should_use_constraints:
    ...
    # 5. Project
    if should_use_constraints and cfg.perform_projection and X_adv is not None:
